...
*** It extracts the data from the JSON recieved from the respective webhook. 
#+NAME: create_issue
#+BEGIN_SRC python
REPO_OWNER =  #Extract from configuration file
REPO_NAME  =  #Extract from configuration file
...
class RepoClone:
  ...
  def repoCloner(): #clone_repo before  
    ...
    (retrun_code, output) = linuxCommandExecuter(command)
    ...
    if return_code == 0:
      ...
      print "Repo is already there, not required  to clone it"
	    ...
	    (return_code, output) = linuxCommandExecuter(next_command)
...
class RepoBuild:
  ...
  def repoBuilder():#build_repo before
    ...
    (return_value, output) = linuxCommandExecuter('cd + ; make')
