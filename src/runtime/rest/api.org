...
#+BEGIN_SRC python
	...
	try:
	    ...
	    current_app.logger.debug("Creating issue label on git repository = %s " % (git_url))
	    r = session.post(git_api_url, json.dumps(issue))
	    if r.status_code == 201:
...
class TesteCreateLabel(unittest.TestCase):
	...
	app = self.create_app()
	    ...
	    org_name = git.construct_org_name(self.git_url)
	    label = "tests_passed"
	    DEL_URL = "%s/%s/%s/labels/%s" % (GIT_API, org_name, repo_name, label)
	    session = requests.Session()
	    session.auth = (USERNAME, PASSWORD)
	    print DEL_URL
    ...
    def test_create_git_label(self):
        ...
        with app.app_context():
            git = GitCommands()
	    label = "tests_passed"
	    self.assertEqual(git.create_label(self.git_url, label), True)
...
def docsPusher(git_url, repo_name):
    ...
    try:
	    ...
	    ret_status = git.makeGithubIssue(git_url, title, body, assignee=None, milestone=None, labels=labels)
...
def deploy_repo():
	...
	try:
		...
		if r.text == "Test failed: See log file for errors":
		    ...
		    if not git.git_label_exists(git_url, labels[0]):
	                git.create_label(git_url, labels[0])
		    ret_status = git.makeGithubIssue(git_url, title, body=str(message), assignee=None, milestone=None, labels=labels)
		    abort(500, message)
		else:
		    ...
		    if not git.git_label_exists(git_url, labels[0]):
	                git.create_label(git_url, labels[0])
		    ret_status = git.makeGithubIssue(git_url, title, body=str(data), assignee=None, milestone=None, labels=labels)
