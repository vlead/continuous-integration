...
class RepoBuild:
  ...
  def repoBuilder():#build_repo before
    ...
    if return_value == 0:
      make_github_issue('Build ran successfully', 'Body text', 'assigned_user', 3, ['build_passed'])#change function name
	    current_app.logger.debug("make ran successfully")
	    return True
    
    else:
      make_github_issue('Failed to run build', 'Body text', 'assigned_user', 3, ['build_failed'])#arguments doubt?
