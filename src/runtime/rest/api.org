...
** REST API to build the souces of github repository
   ...
   whenever a new commit is pushed onto the repository. It clones the
   repository if not present and then builds it. If already present it
   pulls the changes and then builds it.
...
** REST API to push the docs into doc server
   ...
   pushing the documents generated in the docs folder into doc server
