...
** Clone repository
   This class contains a function to clone the github reposirtory if
   repository already exists it pulls the code
...
** Build the project
   - This Class contains the function to build the repository and if
     build is failed/success it creates the issue and assigns label to
     it.
#+NAME: build_repo
#+BEGIN_SRC python
...
** Execute shell commands
   This function executes the linux command and returns the output and
   return code.
#+NAME: exec_commands
#+BEGIN_SRC python
...
** Extract git repository name 
   This function takes github repository url as input argumentand
   computes repository name and returns it
#+NAME: get_repo_name
#+BEGIN_SRC python

def repoNameExtacter(repo_url): #extract_repo_name before
    try:
       repo_name = repo_url.split("/")[-1]
       author_name = repo_url.split("/")[-2]
       current_app.logger.debug("repo_name %s, author_name = %s" % (repo_name, author_name))
       return (repo_name, author_name)
...
** Extract git author name
   This function takes github repository url as input argument and
   computes the author (organization) name and returns it.

#+NAME: get_repo_author
#+BEGIN_SRC python

def repoAuthorNameExtacter(url):
    try:
       author_name = repo_url.split("/")[-2]
       current_app.logger.debug("author_name %s" % (author_name))
       return author_name
    except Exception as e:
       current_app.logger.error("Exception %s" % (str(e)))
       raise str(e)
...
** Create issue on github repository
   - It extracts the data from the JSON recieved from the respective
     webhook.
#+NAME: create_issue
#+BEGIN_SRC python

REPO_OWNER = None
def makeGithubIssue(repo_author, repo_name, title, body=None, assignee=None, milestone=None, labels=None):
    url = 'https://api.github.com/repos/%s/%s/issues' % (repo_author, repo_name)
    if (repo_author != "vlead") or (repo_author != "virtual-labs"):
        abort(401, "Unauthorized repository")
...
** Push docs to doc server 
   - This utility function is used to push the docs created after
     running the make file onto the docs folder.
...
class TestWebHook(TestCase):
	...
	response = self.client.post("/ci_hook", data=json.dumps(payload),
				 headers=headers)
...
#+END_EXAMPLE    
  ...
  - Example:
#+BEGIN_EXAMPLE
 http://localhost:5000/build_sources
#+END_EXAMPLE
*** Implementation   
#+NAME: ci_hook
#+BEGIN_SRC python
@api.route("/build_sources", methods = ['POST', 'GET'])
def ci_hook():
...
#+BEGIN_SRC python
class TestBuildWebHook(TestCase):
    TESTING = True
    def create_app(self):
	app = create_app(config)
	return app
...
class TestClone(TestCase):
	...
	def testCiClone(self):
		print "Unit testing the cloning feature"

	        url = "https://github.com/vlead/continuous-integration.git"
	        rc = repoClone(url)
...
class TestBuild(TestCase):
	...
	def testCiBuild(self):
		print "Unit testing the build feature"
	        url = "https://github.com/vlead/continuous-integration.git"
	
	        rb = repoBuild(url)
	        self.asserEqual(rb.repoBuilder(), True)
