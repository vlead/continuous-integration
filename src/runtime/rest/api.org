...
*** get_git_clone_loc
  ...
  Function reads the =git_config= file and returns the
  =git_clone_location=
...
class TestGetGitLoc(unittest.TestCase):
    ...
    def setUp(self):
        self.create_app()
        self.git_url = "https://github.com/vlead/ovpl"	
    ...
    def test_get_git_loc(self):
        ...
        clone_loc = self.git.get_git_clone_loc()
...
*** construct_repo_name
  :PROPERTIES:
  :CUSTOM_ID: get_repo_name_ref
  :END:    
  
  Function reads the =git_url= and returns the =repository name=
...
class TestConstructRepo(unittest.TestCase):
    ...
    def setUp(self):
        self.create_app()
        self.git_url = "https://github.com/vlead/ovpl"
	self.git_url1 = "https://github.com/ksripathi/lab-data-service"
    ...
    def test_construct_repo_error(self):
        print "test_construct_repo"
        repo_name = self.git.construct_repo_name(self.git_url)
	repo_name1 = self.git.construct_repo_name(self.git_url1)
        self.assertNotEqual(repo_name, repo_name1)
...
*** construct_org_name
  :PROPERTIES:
  :CUSTOM_ID: get_repo_org_ref
  :END:    
  
  Function reads the =git_url= and returns the =repository organiation name=
...
class TestConstructOrg(unittest.TestCase):
	...
	org_name = self.git.construct_org_name(self.git_url)
        org_name1 = self.git_url.split("/")[-2]
	self.assertEqual(org_name, org_name1)

    def test_construct_org_error(self):
        print "test_construct_org_error"
	org_name = self.git.construct_org_name(self.git_url)
	org_name1 = self.git.construct_org_name(self.git_url1)
	self.assertNotEqual(org_name, org_name1)
...
*** repo_exists
  ...
  :END:    
  Function checks for existed repository on =git_clone_location=
  directory
...
#+BEGIN_SRC python
	...
	try:
	    ...
	    current_app.logger.debug("Exception %s" % str(e))
	    return False
#+END_SRC
**** Test Cases
...
class TestRepoExist(unittest.TestCase):
    ...
    def tearDown(self):
        repo_path = self.git.git_clone_loc + self.git.construct_repo_name(self.git_url)
        shutil.rmtree(repo_path)

    def test_repo_exist(self):
        print "test_repo_exist"
        repo_name = self.git.construct_repo_name(self.git_url)
        self.git.clone_repo(self.git_url, repo_name)
        self.assertEqual(self.git.repo_exists(repo_name), True)
    """
    def test_repo_exist_error(self):
        print "test_repo_exist_error"
        repo_name = self.git.construct_repo_name(self.git_url)
	self.assertEqual(self.git.repo_exists(repo_name), False)
    """
#+END_SRC


*** clone_repo
    Function clones the repository into =git_clone_location= directory
**** implementation
#+NAME: repo_clone 
#+BEGIN_SRC python
...
*** pull_repo
**** implementation
    Function pulls the repository on =git_clone_location= directory
#+NAME: repo_pull
#+BEGIN_SRC python
...
class TestRepoPull(unittest.TestCase):
    ...
    def test_repo_pull(self):
        ...
        repo_name = self.git.construct_repo_name(self.git_url)
        self.git.clone_repo(self.git_url, repo_name)
	self.git.checkout_version(repo_name, version=GIT_BRANCH)
	pull_status = self.git.pull_repo(repo_name, version=GIT_BRANCH)
...
*** checkout_branch
    Function changes the git branch to =specified branch=
**** implementation
#+NAME: repo_version
#+BEGIN_SRC python
...
class TestRepoVersion(unittest.TestCase):
    ...
    def test_repo_version(self):
        ...
        self.git.clone_repo(self.git_url, repo_name)
	self.git.checkout_version(repo_name, version=GIT_BRANCH)
        self.assertEqual(self.git.get_repo_version(repo_name), GIT_BRANCH)

#+END_SRC


*** get_git_branch
    Function returns the current active branch on repository
**** implementation
...
class TestGetRepoVersion(unittest.TestCase):
    ...
    def test_get_repo_version(self):
        ...
        self.git.clone_repo(self.git_url, repo_name)
	self.git.checkout_version(repo_name, version=GIT_BRANCH)
        self.assertEqual(git.get_repo_version(repo_name), GIT_BRANCH)

#+END_SRC


*** get_git_commit_info
    Function returns the latest commit details like =user name= ,
    =commit id= and =commit message= for a given repository and branch
**** implementation
...
class TestGetCommitInfo(unittest.TestCase):
    ...
    def setUp(self):
        self.create_app()
        self.git_url = "https://github.com/vlead/ovpl"	
	self.repo_path = self.git.git_clone_loc + self.git.construct_repo_name(self.git_url)
    def tearDown(self):
    ...
    def test_get_commit_info(self):
        ...
        self.git.clone_repo(self.git_url, repo_name)
	self.git.checkout_version(repo_name, version=GIT_BRANCH)
	self.git.get_commit_info(repo_name)
        self.assertEqual(self.git.get_repo_version(repo_name), GIT_BRANCH)
...
*** create_issue
  ...
  :CUSTOM_ID: git_issue
  :END:    
  Function creates the github issue on given =github_url= with title,
  body, label(s), milestone and assignee(s).
...
#+BEGIN_SRC python
	...
	try:
		...
		current_app.logger.debug("Issue id = %s" % (issue_id))
...
class TesteCreateGitIssue(unittest.TestCase):
	...
	repo_name = self.git.construct_repo_name(self.git_url)
	self.git.clone_repo(self.git_url, repo_name)
	commit_info = self.git.get_commit_info(repo_name)
	alert_msg = "Failed to build sources details are here"
	body = "`%s` **Author Name :** %s, **Last Commit :** ,%s **Commit Date :** %s, \
	       **Last Commit Message :** %s, **Branch :** %s" % \
	       (alert_msg, commit_info['author_name'], commit_info['last_commit'], \
	       commit_info['commit_date'], commit_info['last_commit_msg'], \
	       commit_info['branch'])
	if not self.git.git_label_exists(self.git_url, labels[0]):
	   self.git.create_label(self.git_url, labels[0])
	(ret_status, issue_id) = self.git.makeGithubIssue(self.git_url, title, body, assignee=None, milestone=None, labels=labels)
...
*** create_issue_label
  ...
  :CUSTOM_ID: git_label
  :END:    
  Function creates the =label= for given repository url.

**** Implementation
#+NAME: create_label
#+BEGIN_SRC python

    def create_label(self, git_url, label):
	repo_name = self.construct_repo_name(git_url)
	repo_org = self.construct_org_name(git_url)
	git_api_url = '%s/%s/%s/labels' % (GIT_API, repo_org, repo_name)
	current_app.logger.debug("Git REST API URL : %s" % (git_api_url))
	...
	try:
	    ...
	    current_app.logger.debug("Creating issue label=%s on git repository = %s " % (issue, git_url))
	    r = session.post(git_api_url, json.dumps(issue))
	    if r.status_code == 201:
		current_app.logger.debug("Successfully created Issue label with name : %s on repository = %s" % (label, git_url))
		return True
	    else:
		current_app.logger.error("Could not creat label : %s on repository = %s" % (label, git_url))
		current_app.logger.error("Response : %s" % r.content)
		return False
	except Exception as e:
	    current_app.logger.error("Exception : %s" % str(e))
...
class TesteCreateLabel(unittest.TestCase):
    ...
    def setUp(self):
        self.create_app()
        self.git_url = "https://github.com/ksripathi/test-project1"
	self.label = "tests_passed"

    def tearDown(self):
        repo_name = self.git.construct_repo_name(self.git_url)
	org_name = self.git.construct_org_name(self.git_url)
	DEL_URL = "%s/%s/%s/labels/%s" % (GIT_API, org_name, repo_name, self.label)
	session = requests.Session()
	session.auth = (USERNAME, PASSWORD)
	session.delete(DEL_URL)

    def test_create_git_label(self):
        print "test_create_git_label"
	self.assertEqual(self.git.create_label(self.git_url, self.label), True)
...
*** git_label_exists
  ...
  :END:    
   ...
   Function checks for existed =label= for given repository url
**** Implementation
#+NAME: git_label_exists
#+BEGIN_SRC python

    def git_label_exists(self, git_url, label):
	repo_name = self.construct_repo_name(git_url)
	repo_org = self.construct_org_name(git_url)
	git_api_url = '%s/%s/%s/labels/%s' % (GIT_API, repo_org, repo_name, label)
	current_app.logger.debug("Git REST API URL : %s" % (git_api_url))
	try:
	    r = requests.get(git_api_url)
	    current_app.logger.debug("status code %s " %(r.status_code))
	    if r.status_code == 200:
	        current_app.logger.debug("Label exists : %s" % (label))
		return True
	    elif r.status_code == 404:
                current_app.logger.debug("Label not exists : %s" % (label))
	        return False
	except Exception as e:
	    current_app.logger.error("Exception : %s" % str(e))
	    raise str(e)
...
**** Test Cases
#+NAME: test_git_label_exists
#+BEGIN_SRC python

class TesteLabelExists(unittest.TestCase):
    ...
    def test_git_label_exists(self):
        print "test_git_label_exists"
    	self.git.create_label(self.git_url, self.label)
	self.assertEqual(self.git.git_label_exists(self.git_url, self.label), True)

    def test_git_label_not_exists(self):
        print "test_git_label_not_exists"
	self.assertEqual(self.git.git_label_exists(self.git_url, self.label1), False)
...
*** implementation
   - This is a utility function which executes the linux command and
     returns status code to identify the success or failiure of
     execution of the respective command on the machine.
...
class TestExecuteCmd(unittest.TestCase):
    ...
    def test_exec_shell_cmd(self):
        print "test_exec_shell_cmd"
	command = "ls -l"
	(status_code, output) = execute_command(command)
...
** Build the repository
  ...
  :END:    
 - This class contains the function which builds the sources from
   github repository.
...
** Push docs to doc server 
  ...
  This utility function pushes the literate documentation of a
  lab/service which are generated during build into a document server
  which is hosted on staging environment.
  
*** Implementation
#+NAME: push_docs
#+BEGIN_SRC python

def docsPusher(git_url, repo_name):
    git = GitCommands()
    ...
    copy_command = "rsync -avz --progress " + SRC_DIR + " " + USER + "@" + IP_ADDRESS + ":" + DEST_DIR + "/" + repo_name
    current_app.logger.debug("copy command = %s" % copy_command)
    try:    
        (ret_code, output) = execute_command(copy_command)
        if ret_code == 0:
	    """
	    #body = git.get_commit_info(repo_name)
	    body = "http://%s/%s" % (IP_ADDRESS, repo_name)
	    title = "Published docs on doc server"
	    labels = ["docs_published"]		   
	    if not git.git_label_exists(git_url, labels[0]):
	        git.create_label(git_url, labels[0])
	    (ret_status, issue_id) = git.makeGithubIssue(git_url, title, body, assignee=None, milestone=None, labels=labels)
	    """
...
class TestPushDocs(unittest.TestCase):
	...
	repo_build.repoBuilder()
        self.assertEqual(docsPusher(self.git_url, repo_name), True)
...
** Run test suite
  ...
  :CUSTOM_ID: run_tests
  :END:    
  This class contains functions to create the =test environment= and
...
class TestRun:
	...
	git = GitCommands()
	try:
	    commit_info = git.get_commit_info(self.repo_name)
	    alert_msg = "Successfully ran all the test cases as on commit details :"
	    body = "`%s` **Author Name :** %s, **Last Commit :** ,%s **Commit Date :** %s, \
	           **Last Commit Message :** %s, **Branch :** %s" % \
		   (alert_msg, commit_info['author_name'], commit_info['last_commit'], \
		   commit_info['commit_date'], commit_info['last_commit_msg'], \
		   commit_info['branch'])
	    #body = git.get_commit_info(self.repo_name)
		...
		labels = ["ran_test_cases"]
		if not git.git_label_exists(self.git_url, labels[0]):
		    git.create_label(self.git_url, labels[0])
		git.makeGithubIssue(self.git_url, title, body, assignee=None, milestone=None, labels=labels)
	        return True
	except Exception as e:
	    current_app.logger.error("Couldn't Ran test file = %s" % (test_file))
	    alert_msg = "Failed to ran all the test cases as on commit details :"
...
class TestCreateTestEnv(unittest.TestCase):
    ...
    def setUp(self):
        self.create_app()
	self.git = GitCommands()
        self.git_url = "https://github.com/ksripathi/test-project"
        self.git_clone_loc = self.git.git_clone_loc
    ...
    """
    def test_run_test_suit(self):
        print "test_test_suit"
        repo_name = self.git.construct_repo_name(self.git_url)
        self.git.clone_repo(self.git_url, repo_name)
        repo_build = RepoBuild(self.git_url)
	repo_build.repoBuilder()
...
** REST API to build the souces of github repository
   ...
   - This API clone/pull the repository and then builds the
     sources. 

   - If build is failed to generate it creates an issue on respective
     repository with title =Build sources= 

   - Also it assigns label called =build_failed= otherwise
     =build_success=.
     
   - It attaches the last commit details like =user name= , =commit
     id= , =commit message= and =date of commit= into the created
     issue as a body content which causes the build failed/success
...
def build_sources():
	...
	try:
	    ...
	    git_url = repo_data['repository']['html_url']
            git_branch = repo_data['ref']
            if "https://github.com/ksripathi" not in str(git_url):
                current_app.logger.debug("Unauthorized webhook git url: %s" % (git_url))
                abort(401, "Unauthorized webhook request")
            current_app.logger.debug("Git branch: %s" % (git_branch))
...
** REST API to push the docs into doc server
   ...
   - Also it runs all the Test Cases by creating the test environment.
     even if single test case is failed to run it creates the issue
     with titled =coudn't run test case <name>= and assigns the label
   
   - If Test Cases or success/failed to run it creates the label
     =successfully_ran_tests= and =failed_to_run_tests= respectively
     
   - It attaches the last commit details (like =user name= , =commit
     id= , =commit message= and =date of commit= into the created
     issue as a body content which causes the Test Cases
     failed/success to run) and =URL= of doc server where the
     documents are pushed/hosted into
...
def ci_push_docs():
    ...
    tr.createTestEnv()
    if tr.runTestSuit():
        status = docsPusher(git_url, repo_name)  
	if status:
	    title = "Docs Published"
            labels = ["docs_published"]
            commit_info = git.get_commit_info(repo_name)
            msg = "Successfully copied docs to doc-server:"
            body = "`%s` **URL :** http://%s/%s" %  (msg, IP_ADDRESS, repo_name)
            if not git.git_label_exists(git_url, labels[0]):
                git.create_label(git_url, labels[0])
            (ret_status, issue_id) = git.makeGithubIssue(git_url, title, body, assignee=None, milestone=None, labels=labels)
...
** REST API to deploy the service/lab using ADS
  ...
  :END:    
   ...
   - It attaches the last commit details (like =user name= , =commit
     id= , =commit message= and =date of commit= into the created
     issue as a body content which causes the build failed/success)
     and =URL= of doc server where the documents are pushed/hosted into

   - After build & test suite ran successfully this repository will be
     deployed on =staging-server= using ADS service's =RESTfull APIs=.
...
def deploy_repo():
	...
	data = {'lab_id': lab_id, 'lab_src_url': lab_url, 'version': tag, \
		'key' : ADS_SECRET_KEY, "current_user" : "ci-service"}
	...
	try:
		...
		if r.text == "Test failed: See log file for errors":
		    ...
		    title = "Error in deploying app on testing server"
		    labels = ["deploy_failed"]		   
		    if not git.git_label_exists(git_url, labels[0]):
	                git.create_label(git_url, labels[0])
		    (ret_status. issue_id) = git.makeGithubIssue(git_url, title, body=str(message), assignee=None, milestone=None, labels=labels)
		    abort(500, message)
		else:
		    data['url'] = "http://" + r.text                      
		    title = "Successfully Deployed on Testing Server"
		    labels = ["deployed"]		      
		    if not git.git_label_exists(git_url, labels[0]):
	                git.create_label(git_url, labels[0])
		    (ret_status, issue_id) = git.makeGithubIssue(git_url, title, body=str(data), assignee=None, milestone=None, labels=labels)
