...
** REST API to deploy the service/lab using ADS
   ...
   This API will be called if registered with the webhook.It provides
   an additional functionality over the prexisiting functionilties of
   ci_hook and push_docs APIS of passing the the repository URL to the
   ADS controller so as to invoke the auto depolyement process.
