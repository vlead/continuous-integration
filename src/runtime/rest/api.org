...
** REST API to build the souces of github repository
   This API if registered with in a respective webhook will be invoked 
   whenever a somwthing new is pushed onto the repository.It clones the repository
   if not present and then builds it.If already present it pulls the changes and then
   builds it.
...
*** Integration Test for the above API.
...
This API is registered in the webhook provides an additonal functionality of 
pushing the documents generated in the docs folder along with the already existing 
...
** REST API to deploy the service/lab using ADS
 ...
 This API will be called if registered with the webhook.It provides an additional 
 functionality over the prexisiting functionilties of ci_hook and push_docs APIS of passing the  
 the repository URL to the ADS controller so as to invoke the auto depolyement process. 
