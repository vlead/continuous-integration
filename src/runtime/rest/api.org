...
class RepoClone:
	...
	repo_name = repoNameExtacter(self.git_url)
...
class RepoBuild:
    ...
    def repoBuilder(self, repo_name):#build_repo before
	...
	if ret_code == 0:
	    ...
	    ret_status = makeGithubIssue(repo_name, 'Build ran successfully', \
	    ...
	    current_app.logger.debug("Creating github issue")
	    ret_status = makeGithubIssue(repo_name, 'Failed to run build', \
                                           'Body text', 'assigned_user', 3, ['build_failed'])
	    if ret_status:
		current_app.logger.debug("Created github issue")
		return False
...
REPO_OWNER = None
def makeGithubIssue(repo_name, title, body=None, assignee=None, milestone=None, labels=None):
...
def ci_hook():
	...
	git_url = repo_data['repository']['html_url']
        global REPO_AUTHOR
	REPO_AUTHOR = repo_data['repository']['owner']['login']
	current_app.logger.debug("Git repository author name : %s" % (REPO_AUTHOR))
