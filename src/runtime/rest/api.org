...
class TestExecuteCmd(unittest.TestCase):
	...
	command = "http://github.com/vlead/ovpl.git"
	self.git.clone_repo(command, "ovpl")
        self.assertEqual(self.git.repo_exists("ovpl"), True)
...
class TestRun:
	...
	count = 0
	try:
	    body = self.get_commit_info(self.repo_name)
		...
		if not self.git.git_label_exists(self.git_url, labels[0]):
		    self.git.create_label(self.git_url, labels[0])
		self.git.makeGithubIssue(self.git_url, title, body, assignee=None, milestone=None, labels=labels)
		return True
	except Exception as e:
