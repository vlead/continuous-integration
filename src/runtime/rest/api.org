...
class TesteExecuteCommand(TestCase):
...
class TesteExtractRepo(TestCase):
	...
	self.asserEqual(repo_name, "continuous-integration")
        self.asserEqual(repo_org, "vlead")
...
class TesteRepoExist(TestCase):
	...
	url = "https://github.com/vlead/continuous-integration.git"
        rc = repoClone(url)
        (repo_name, repo_org) = repoNameExtacter(url)
	self.asserEqual(isRepoExist(repo_name), True)
