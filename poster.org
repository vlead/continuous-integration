AIM : To ensure evolution of software and solicit contribution from folks across the world, continuous integration is to be built around each lab. A cycle for integration should involve building the code, running the tests,notifying the users and deployment.
...
METHODOLOGY:Our basis of the whole implementaion was to build a microservice which could be scaled over time with the requirement.One of the best ways to implement a microservice is using flask in python.The Continuous Integration service was provided in the form of APIs to the conritbutors/ developers.The functionality provided by Github webhooks were exploited to invoke these APIs whenever a change was made to the repo.The JSON payload
